...
function updateToggle(){const blocked=document.body.classList.contains('motion-paused')||reduced.matches;toggle.textContent=playing&&!blocked?'Ⅱ':'▶';toggle.setAttribute('aria-label',playing&&!blocked?'暂停项目动画':'播放项目动画');toggle.setAttribute('aria-pressed',String(playing&&!blocked));toggle.disabled=blocked;toggle.title=blocked?'可使用页面右上角动效开关；系统减少动态效果偏好生效时保持静态。':''}
function loop(time){
